...
#### 23. 중성화 여부 파악 
```sql
-- 1. IF 문을 SELECT에서 사용가능
-- 2. CASE WHEN THEN ELSE END로 사용가능
...
ORDER BY ANIMAL_ID
```
#### 24. 오랜 기간 보호한 동물, 보호 기간이 가장 길었던 동물 두 마리의 아이디와 이름 
```sql
-- LEFT JOIN
...
SELECT ANIMAL_ID, OUTS.NAME
FROM ANIMAL_INS AS INS
RIGHT JOIN ANIMAL_OUTS AS OUTS USING(ANIMAL_ID)
ORDER BY (OUTS.DATETIME - INS.DATETIME) DESC
LIMIT 2
```
...
-- %Y: 2021 같이 전체적으로 표시 / %y: 21 같이 뒤 두자리만 표시
-- %M: 영어로 달을 표시 / %m: 숫자로 표시
-- %D: 서수로 표시 / %d: 숫자로 표시
...
```sql
SELECT BRANCH_ID, ROUND(AVG(SALARY), 2) -- 급여 평균
-- SELECT BRANCH_ID, SUM(SALARY), 2 급여합
FROM EMPLOYEES
GROUP BY BRANCH_ID
ORDER BY BRANCH_ID
```
